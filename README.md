...
- **inventory-&lt;env&gt;.ini**: Defines the target hosts under the `[ec2_instances]` group. This file is automatically generated by Terraform (via the `local_file` resource in each environment's `main.tf`) from the `ansible_inventory.tpl` template, populated with the public IPs of the provisioned EC2 instances.
- **site.yml**: Playbook that installs Docker on the instances (uses the `docker_engine` role).
- **site-nginx.yml**: Playbook that installs and configures Nginx to serve the static site (uses the `webserver` role).
- **roles/**: Contains reusable Ansible roles.
    - **docker_engine**: Installs Docker Engine and adds `ec2-user` to the `docker` group.
